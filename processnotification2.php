...
if(!($result = $appdb->query("select * from tbl_smsoutbox where smsoutbox_sendfb>0 and smsoutbox_sendfbstatus=1 order by smsoutbox_id asc limit 5"))) {
	json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
	die;
}
...
if(!empty($notifications)) {
	...
	print_r(array('outbox facebook notification'=>$notifications));
