...
function modemInit($dev=false,$mobileNo=false,$ip='',$baud=115200,$simMenu=false) {
	...
	if(!($result=$appdb->query("select * from tbl_sim where sim_disabled=0 and sim_deleted=0 and sim_online=1 and sim_number='$mobileNo'"))) {
		return false;
	}

	if(!empty($result['rows'][0]['sim_id'])) {
		$simMenu = !empty($result['rows'][0]['sim_menu']) ? true : false;
	} else return false;
