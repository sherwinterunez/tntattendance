...
define ("DEVICE_OPENED", 2);
	...
	class APP_app_messaging extends APP_Base_Ajax {
		...
		function _form_messagingmaincomposexxx($routerid=false,$formid=false,$flag=false) {
			global $applogin, $toolbars, $forms, $apptemplate, $appdb;
			//pre(array('$this->vars'=>$this->vars));
					...
					if($this->vars['post']['method']=='messagingresend') {
						...
						if(!empty($this->vars['post']['rowids'])) {
							...
							if(!empty($arowid)) {
								...
								if(!($result = $appdb->update("tbl_smsoutbox",array('smsoutbox_status'=>1,'smsoutbox_failedcount'=>0),'smsoutbox_id in ('.$rowids.')'))) {
									json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
									die;
								}
						...
						if(!empty($this->vars['post']['rowid'])&&is_numeric($this->vars['post']['rowid'])) {
							...
							if(!($result = $appdb->update("tbl_smsoutbox",array('smsoutbox_status'=>1,'smsoutbox_failedcount'=>0),'smsoutbox_id='.$this->vars['post']['rowid']))) {
								json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
								die;
							}
...
/////
						...
						if(!empty($result['rows'][0]['count'])) {
							$contact_count = $result['rows'][0]['count'];
						}
/////
						...
						if(!empty($result['rows'][0]['smsinbox_id'])) {
							...
							if(!empty($contact_count)) {
								$retval['contact'] = number_format($contact_count);
							}
						}
					} else
...
/////
					...
					if($this->post['table']=='tonumber') {
						...
						if(!empty($result['rows'][0]['contact_id'])) {
							...
							foreach($result['rows'] as $k=>$v) {
								$rows[] = array('id'=>$v['contact_id'],'data'=>array(0,$v['contact_number'],$v['contact_nick']));
							}
