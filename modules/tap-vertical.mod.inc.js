...
if(intval($settings_pageautorefreshinterval)>0) {
	...
	$settings_pageautorefreshintervalmilli = 5000;
}
...
var loginForm = [
	...
	{type: "hidden", value: "0", name:"recordid"},
...
srt.refresh = function() {
	...
	var recordid = myForm.getItemValue("recordid");
	...
	postData('/'+settings.router_id+'/refresh/','&unixtime='+unixtime+'&imagesize='+imagesize+'&total='+studentprevTotal,function(data){
		...
		if(typeof data.recordid != 'undefined' ) {
			...
			if(parseInt(data.recordid)>0&&recordid!=parseInt(data.recordid)) {
				console.log('data.recordid',parseInt(data.recordid));
				myForm.setItemValue("recordid",parseInt(data.recordid));
			} else {
				return false;
			}
...
jQuery(document).ready(function($) {
	...
	<?php if(!empty($settings_pageautorefresh)) { ?>
	setInterval(function(){
		srt.refresh();
