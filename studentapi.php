...
*
* Date Created: November 25, 2018 8:36:35PM
*
* Description:
*
...
if(!empty($_POST['opt'])) {
	$topt = strtoupper(trim($_POST['opt']));
	//if($topt=='CREATE'||$topt=='READ'||$topt=='UPDATE'||$topt=='DELETE'||$topt=='UPLOAD') {
	if($topt=='CREATE'||$topt=='UPDATE'||$topt=='DELETE') {
		$opt = $topt;
	} else {
		$ret = array();
...
foreach($lines as $k=>$v) {
  ...
  $content['studentprofile_active'] = !empty($data[12]) ? trim($data[12]) : '1';
	...
	if(trim($content['studentprofile_firstname'])!='') {
		...
		if($opt=='DELETE') {
			...
			$invalid[] = $content['studentprofile_number'];
			continue;
		}
	}
  ...
  if(!empty($data[14])&&!empty($studentprofile_yearlevel)) {
	...
	if($opt=='CREATE') {
		...
		if(!($result = $appdb->insert("tbl_studentprofile",$content,"studentprofile_id"))) {
			if(preg_match('/duplicate key value violates unique constraint/si',$appdb->lasterror)) {
				$duplicate[] = $content['studentprofile_number'];
		...
		if(!empty($result['returning'][0]['studentprofile_id'])) {
			...
			$success[] = $content['studentprofile_number'];
	...
	if($opt=='UPDATE') {
		...
		$xstudentprofile_number = $content['studentprofile_number'];

		unset($content['studentprofile_number']);

		if(!($result = $appdb->update("tbl_studentprofile",$content,"studentprofile_number='".$xstudentprofile_number."'"))) {
			json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror));
			$appdb->rollback();
			die;
		}

		$update[] = $xstudentprofile_number;
	...
	if($opt=='DELETE') {
		...
		if(!($result = $appdb->query("delete from tbl_studentprofile where studentprofile_number='".$content['studentprofile_number']."'"))) {
			json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
			$appdb->rollback();
			die;
		}

		$delete[] = $content['studentprofile_number'];
