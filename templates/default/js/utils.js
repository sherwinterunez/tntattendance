    ...
    if (audio_element.canPlayType){
        for (var i=0; i<arguments.length; i++){
            ...
            if (arguments[i].match(/\.(\w+)$/i))
                source_element.setAttribute('type', srt.audiotypes[RegExp.$1])
...
var ss_newmessage  = ss_soundbits("/audio/you-have-new-message.mp3");
...
function openWindow(obj,cb) {
	...
	//wid += '_'+srt.windowsCtr;

	//srt.windowsCtr = srt.windowsCtr + 1;
