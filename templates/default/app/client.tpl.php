...
<style>
	...
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .client_grid_%formval%,
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .client_grid_%formval% .dhxform_control {
		padding: 0;
	}
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .client_grid_%formval% .dhxform_control .gridbox {
		border: none;
	}
...
<script>
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_resize_%formval%(myWinObj) {
		...
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .client_grid_%formval% .dhxform_container").height(dim[1]-103);
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .client_grid_%formval% .dhxform_container").width(dim[0]-24);
		...
		if(typeof(myWinObj.myGridClients)!='undefined') {
			try {
				myWinObj.myGridClients.setSizes();
			} catch(e) {}
		}
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_%formval%() {
		...
		myTabbar.addTab("tbClientRecords", "Client Records");
		//myTabbar.addTab("tbLoginNotification", "Login Notification");

		myTabbar.tabs("tbClientRecords").setActive();
		...
		var formData2_%formval% = [
			...
			{type: "block", name: "tbClientRecords", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbClientRecords']) ? json_encode($params['tbClientRecords']) : '[]'; ?>},
			/*{type: "label", label: ""}*/
		];
...
///////////////////////////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&method=<?php echo $method; ?>&table=clients&formval=%formval%",
		}, function(ddata,odata){

			if(typeof(myWinObj.myGridClients)!='null'&&typeof(myWinObj.myGridClients)!='undefined'&&myWinObj.myGridClients!=null) {
				try {
					myWinObj.myGridClients.destructor();
					myWinObj.myGridClients = null;
				} catch(e) {
					console.log(e);
				}
			}

			var myGridClients = myWinObj.myGridClients = new dhtmlXGridObject(myForm.getContainer('client_grid'));

			myGridClients.setImagePath("/codebase/imgs/")

			myGridClients.setHeader("#master_checkbox, Seq, ID, Code, Name, Contact Person, Contact Number, Population, URL, Status, Active, Create Stamp, Update Stamp");

			myGridClients.setInitWidths("35,50,50,120,120,120,120,120,120,120,120,120,120");

			myGridClients.setColAlign("center,center,center,left,left,left,left,left,left,left,left,left,left");

			myGridClients.setColTypes("ch,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");

			myGridClients.setColSorting("int,int,int,str,str,str,str,str,str,str,str,str,str");

			myGridClients.attachHeader("&nbsp;,&nbsp;,&nbsp;,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#combo_filter,#combo_filter,#text_filter,#text_filter");

			myGridClients.init();

			myGridClients.attachEvent("onRowDblClicked", function(rowId,cellIndex){
				...
				openWindow(obj, function(winobj,obj){
					...
					myTab.postData('/'+settings.router_id+'/json/', {
						...
						pdata: "routerid="+settings.router_id+"&action=formonly&formid=<?php echo $templatedetailid.'clients'; ?>&module=<?php echo $moduleid; ?>&method=onrowselect&rowid="+rowId+"&formval=%formval%&wid="+obj.wid,
					}, function(ddata,odata){
						if(ddata.toolbar) {
			...
			try {
				myGridClients.parse(ddata,function(){

					<?php if(!($method==$moduleid.'new'||$method==$moduleid.'edit')) { ?>

					myGridClients.forEachRow(function(id){
						//myGridClients.cells(id,1).setDisabled(true);
						//myGridClients.cells(id,2).setDisabled(true);
						//myGridClients.cells(id,3).setDisabled(true);
						//myGridClients.cells(id,4).setDisabled(true);
						//myGridClients.cells(id,5).setDisabled(true);
					});
...
///////////////////////////////////
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>new').onClick = function(id,formval) {
			...
			obj.routerid = settings.router_id;
			obj.action = 'formonly';
			obj.formid = '<?php echo $templatedetailid; ?>clients';
			obj.module = '<?php echo $moduleid; ?>';
			obj.method = id;
			obj.rowid = 0;
			obj.formval = '%formval%';

			obj.title = 'New Client Profile';
