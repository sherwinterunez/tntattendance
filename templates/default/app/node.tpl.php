...
<style>
	...
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .node_grid_%formval%,
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .node_grid_%formval% .dhxform_control {
		padding: 0;
	}
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .node_grid_%formval% .dhxform_control .gridbox {
		border: none;
	}
...
<script>
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_resize_%formval%(myWinObj) {
		...
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .node_grid_%formval% .dhxform_container").height(dim[1]-103);
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .node_grid_%formval% .dhxform_container").width(dim[0]-24);
		...
		if(typeof(myWinObj.myGridNodes)!='undefined') {
			try {
				myWinObj.myGridNodes.setSizes();
			} catch(e) {}
		}
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_%formval%() {
		...
		myTabbar.addTab("tbNodeRecords", "Node Records");
		//myTabbar.addTab("tbLoginNotification", "Login Notification");

		myTabbar.tabs("tbNodeRecords").setActive();
		...
		var formData2_%formval% = [
			...
			{type: "block", name: "tbNodeRecords", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbNodeRecords']) ? json_encode($params['tbNodeRecords']) : '[]'; ?>},
			/*{type: "label", label: ""}*/
		];
...
///////////////////////////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&method=<?php echo $method; ?>&table=nodes&formval=%formval%",
		}, function(ddata,odata){

			if(typeof(myWinObj.myGridNodes)!='null'&&typeof(myWinObj.myGridNodes)!='undefined'&&myWinObj.myGridNodes!=null) {
				try {
					myWinObj.myGridNodes.destructor();
					myWinObj.myGridNodes = null;
				} catch(e) {
					console.log(e);
				}
			}

			var myGridNodes = myWinObj.myGridNodes = new dhtmlXGridObject(myForm.getContainer('node_grid'));

			myGridNodes.setImagePath("/codebase/imgs/")

			myGridNodes.setHeader("#master_checkbox, Seq, ID, Code, Name, URL, VPN IP, VPN Port, Local IP, Status, Active, Create Stamp, Update Stamp");

			myGridNodes.setInitWidths("35,50,50,120,120,120,120,120,120,120,120,120,120");

			myGridNodes.setColAlign("center,center,center,left,left,left,left,left,left,left,left,left,left");

			myGridNodes.setColTypes("ch,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");

			myGridNodes.setColSorting("int,int,int,str,str,str,str,str,str,str,str,str,str");

			myGridNodes.attachHeader("&nbsp;,&nbsp;,&nbsp;,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#combo_filter,#combo_filter,#text_filter,#text_filter");

			myGridNodes.init();

			myGridNodes.attachEvent("onRowDblClicked", function(rowId,cellIndex){
				...
				openWindow(obj, function(winobj,obj){
					...
					myTab.postData('/'+settings.router_id+'/json/', {
						...
						pdata: "routerid="+settings.router_id+"&action=formonly&formid=<?php echo $templatedetailid.'nodes'; ?>&module=<?php echo $moduleid; ?>&method=onrowselect&rowid="+rowId+"&formval=%formval%&wid="+obj.wid,
					}, function(ddata,odata){
						if(ddata.toolbar) {
			...
			try {
				myGridNodes.parse(ddata,function(){

					<?php if(!($method==$moduleid.'new'||$method==$moduleid.'edit')) { ?>

					myGridNodes.forEachRow(function(id){
						//myGridNodes.cells(id,1).setDisabled(true);
						//myGridNodes.cells(id,2).setDisabled(true);
						//myGridNodes.cells(id,3).setDisabled(true);
						//myGridNodes.cells(id,4).setDisabled(true);
						//myGridNodes.cells(id,5).setDisabled(true);
					});
...
///////////////////////////////////
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>new').onClick = function(id,formval) {
			...
			obj.routerid = settings.router_id;
			obj.action = 'formonly';
			obj.formid = '<?php echo $templatedetailid; ?>nodes';
			obj.module = '<?php echo $moduleid; ?>';
			obj.method = id;
			obj.rowid = 0;
			obj.formval = '%formval%';

			obj.title = 'New Node Profile';
