...
<style>
	...
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .monitor_grid_%formval%,
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .monitor_grid_%formval% .dhxform_control {
		padding: 0;
	}
	#<?php echo $wid; ?> #<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .monitor_grid_%formval% .dhxform_control .gridbox {
		border: none;
	}
...
<script>
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_monitor_%formval%() {
		...
		var tdata = myWinToolbar.getToolbarData('<?php echo $moduleid; ?>refresh');

		if(typeof(myWinObj.form)!='undefined') {
			myWinToolbar.getToolbarData('<?php echo $moduleid; ?>refresh').onClick.apply(myWinToolbar,['<?php echo $moduleid; ?>refresh','%formval%']);
		}

		/*setTimeout(function(){
			<?php echo $wid.$templatedetailid.$submod; ?>_monitor_%formval%();
		}, 1000*60*1);*/
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_resize_%formval%(myWinObj) {
		...
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .monitor_grid_%formval% .dhxform_container").height(dim[1]-103);
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .monitor_grid_%formval% .dhxform_container").width(dim[0]-24);
		...
		if(typeof(myWinObj.myGridMonitors)!='undefined') {
			try {
				myWinObj.myGridMonitors.setSizes();
			} catch(e) {}
		}
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_%formval%() {
		...
		myTabbar.addTab("tbMonitorRecords", "Monitoring");
		//myTabbar.addTab("tbLoginNotification", "Login Notification");

		myTabbar.tabs("tbMonitorRecords").setActive();
		...
		var formData2_%formval% = [
			...
			{type: "block", name: "tbMonitorRecords", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbMonitorRecords']) ? json_encode($params['tbMonitorRecords']) : '[]'; ?>},
			/*{type: "label", label: ""}*/
		];
...
///////////////////////////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&method=<?php echo $method; ?>&table=monitors&formval=%formval%",
		}, function(ddata,odata){

			if(typeof(myWinObj.myGridMonitors)!='null'&&typeof(myWinObj.myGridMonitors)!='undefined'&&myWinObj.myGridMonitors!=null) {
				try {
					myWinObj.myGridMonitors.destructor();
					myWinObj.myGridMonitors = null;
				} catch(e) {
					console.log(e);
				}
			}

			var myGridMonitors = myWinObj.myGridMonitors = new dhtmlXGridObject(myForm.getContainer('monitor_grid'));

			myGridMonitors.setImagePath("/codebase/imgs/")

			myGridMonitors.setHeader("#master_checkbox, Seq, ID, Code, Name, Queued, Waiting, Sending, Sent, Failed, Status, Active, Update Stamp");

			myGridMonitors.setInitWidths("35,50,50,150,*,90,90,90,90,90,100,100,120");

			myGridMonitors.setColAlign("center,center,center,left,left,left,left,left,left,left,left,left");

			myGridMonitors.setColTypes("ch,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");

			myGridMonitors.setColSorting("int,int,int,str,str,str,str,str,str,str,str,str,str");

			myGridMonitors.attachHeader("&nbsp;,&nbsp;,&nbsp;,#text_filter,#combo_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#combo_filter,#combo_filter,#text_filter");

			myGridMonitors.init();

			<?php /*myGridMonitors.attachEvent("onRowDblClicked", function(rowId,cellIndex){
				...
				openWindow(obj, function(winobj,obj){
					...
					myTab.postData('/'+settings.router_id+'/json/', {
						...
						pdata: "routerid="+settings.router_id+"&action=formonly&formid=<?php echo $templatedetailid.'monitors'; ?>&module=<?php echo $moduleid; ?>&method=onrowselect&rowid="+rowId+"&formval=%formval%&wid="+obj.wid,
					}, function(ddata,odata){
						if(ddata.toolbar) {
			...
			try {
				myGridMonitors.parse(ddata,function(){

					<?php if(!($method==$moduleid.'new'||$method==$moduleid.'edit')) { ?>

					myGridMonitors.forEachRow(function(id){
						//myGridMonitors.cells(id,1).setDisabled(true);
						//myGridMonitors.cells(id,2).setDisabled(true);
						//myGridMonitors.cells(id,3).setDisabled(true);
						//myGridMonitors.cells(id,4).setDisabled(true);
						//myGridMonitors.cells(id,5).setDisabled(true);
					});
...
///////////////////////////////////
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>new').onClick = function(id,formval) {
			...
			obj.routerid = settings.router_id;
			obj.action = 'formonly';
			obj.formid = '<?php echo $templatedetailid; ?>monitors';
			obj.module = '<?php echo $moduleid; ?>';
			obj.method = id;
			obj.rowid = 0;
			obj.formval = '%formval%';

			obj.title = 'New Monitor Profile';
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>cancel').onClick = myWinToolbar.getToolbarData('<?php echo $moduleid; ?>refresh').onClick = function(id,formval) {
			...
			srt.dummy.apply(this,[myWinToolbar.getToolbarData('<?php echo $moduleid; ?>refresh')]);
		...
		myWinObj.timeout = setTimeout(function(){
			<?php echo $wid.$templatedetailid.$submod; ?>_monitor_%formval%();
		}, 1000*60*1);
