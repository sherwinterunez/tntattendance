...
function textAssist(ip) {
  ...
  console.log("textassist.php "+ip+" running...");

  phpfpm.run('textassist.php?ip='+ip, function(err, output, phpErrors)
  {
      if (err == 99) console.error('PHPFPM server error');
...
function relayRestart(ip) {
  ...
  console.log("relay.php "+ip+" running...");

  phpfpm.run('relay.php?ip='+ip, function(err, output, phpErrors)
  {
      if (err == 99) console.error('PHPFPM server error');
...
function processPushNotification() {
  ...
  console.log("processpushnotification.php running...");

  //phpfpm.run('retrieve2.php?dev='+dev+'&sim='+sim+'&ip='+ip, function(err, output, phpErrors)
  phpfpm.run('processpushnotification.php', function(err, output, phpErrors)
  {
      if (err == 99) console.error('PHPFPM server error');
...
function processFace() {
  ...
  console.log("faceprocess.php running...");

  phpfpm.run('faceprocess.php', function(err, output, phpErrors)
  {
      if (err == 99) console.error('PHPFPM server error');
...
function processFbNotification() {

  console.log("processfbnotification.php running...");

  //phpfpm.run('retrieve2.php?dev='+dev+'&sim='+sim+'&ip='+ip, function(err, output, phpErrors)
  phpfpm.run('processfbnotification.php', function(err, output, phpErrors)
  {
      if (err == 99) console.error('PHPFPM server error');
...
function portCheck() {
  ...
  phpfpm.run('portcheck.php', function(err, output, phpErrors)
  {
      ...
      if(obj&&obj.devices&&obj.devices.length>0) {
        ...
        if(obj.textassistips&&obj.textassistips.length>0) {
          textassistips = obj.textassistips;

          for(var j in textassistips) {
            textAssist(textassistips[j]);
          }
        }

        if(obj.relayips&&obj.relayips.length>0) {
          relayips = obj.relayips;

          for(var j in relayips) {
            relayRestart(relayips[j]);
          }
        }
